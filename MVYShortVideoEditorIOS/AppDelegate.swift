...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        let inputViewController = MVYInputViewController.init()
        
        AppDelegate.clearCache()
        
        rootViewController.pushViewController(inputViewController, animated: false)
...
extension AppDelegate {
    class func clearCache() {
        guard let cachePath = NSSearchPathForDirectoriesInDomains(.cachesDirectory, .userDomainMask, true).first,
            let files = FileManager.default.subpaths(atPath: cachePath) else { return }
